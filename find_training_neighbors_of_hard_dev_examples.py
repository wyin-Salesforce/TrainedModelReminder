...
def main():
    ...
    for idd, valid_dataloader in enumerate(valid_dataloader_list):
        ...
        for _, batch in enumerate(valid_dataloader):
            ...
            with torch.no_grad():
                logits, reps = model(input_ids, input_mask, None, labels=None)

            dev_reps.append(reps)
        ...
        dev_reps = torch.cat(dev_reps, dim=0)
        ...
        assert len(pred_label_ids) == dev_reps.shape[0]
        hit_co = 0
        for k in range(len(pred_label_ids)):
            if pred_label_ids[k] == gold_label_ids[k]:
                ...
                incorrect_pred_co+=1
                print('incorrect_pred_co:', incorrect_pred_co)
...
'''
 CUDA_VISIBLE_DEVICES=0 python -u find_training_neighbors_of_hard_dev_examples.py --task_name rte --do_lower_case --learning_rate 2e-6 --num_train_epochs 100 --per_gpu_train_batch_size 32 --per_gpu_eval_batch_size 64

1, 先把所有trainig examples, dev examples的representations and labels找到; dev的要pred_label, and gold_label
2, 遍历dev,当pred_label != gold_label; search top-100 neighbors (indices)
3, train again on all neighbors
'''
